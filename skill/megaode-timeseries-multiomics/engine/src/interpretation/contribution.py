...
def compute_contributions(
    ...
    for name in names:
        ...
        grad, signed_grad = _finite_difference(model, bundle)
        ...
        if coef is not None:
            signed_maps["coefficient"] = coef
        signed_maps["gradient"] = signed_grad
        ...
        for j, target in enumerate(bundle.y_features):
            ...
            for i, src in enumerate(bundle.x_features):
                ...
                for method, mat in maps.items():
                    ...
                    signed_mat = signed_maps.get(method)
                    if signed_mat is not None:
                        signed_value = float(signed_mat[j, i])
                        rec["signed_score"] = signed_value
                        if method == "gradient":
                            rec["signed_gradient"] = signed_value
                        elif method == "coefficient":
                            rec["signed_coefficient"] = signed_value
                    records.append(rec)
    if shap_ok:
