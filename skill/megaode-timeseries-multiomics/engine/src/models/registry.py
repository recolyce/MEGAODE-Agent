...
PACKAGED_GENERATED_DIR = Path(__file__).resolve().parent / "generated"
DEFAULT_MODELS = list(MODELS)


def generated_dir() -> Path:
    """Write destination for this-run generated models.

    Prefer ``TMO_GENERATED_DIR`` so Skill install trees stay read-only.
    """
    override = (os.environ.get("TMO_GENERATED_DIR") or "").strip()
    if override:
        return Path(override)
    return PACKAGED_GENERATED_DIR


def generated_scan_dirs() -> list[Path]:
    dirs: list[Path] = []
    if PACKAGED_GENERATED_DIR.exists():
        dirs.append(PACKAGED_GENERATED_DIR)
    extra = generated_dir()
...
# Backward-compatible alias. Prefer generated_dir() — this Path is the packaged tree.
GENERATED_DIR = PACKAGED_GENERATED_DIR
...
def _load_generated() -> None:
    ...
    for directory in generated_scan_dirs():
