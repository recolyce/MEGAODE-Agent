...
# Portable defaults for Bohrium / BioMaster. TMO_PRIOR_ROOT always wins.
# Do not default to /personal/workspace — that path is not available after Skill upload.
CANDIDATE_PRIOR_ROOTS = (
    Path("/share/TMO-agent-prior"),
    Path("/bohr-workspace/cache/megaode-prior"),
)
DEFAULT_PRIOR_ROOT = CANDIDATE_PRIOR_ROOTS[0]


def prior_root() -> Path:
    override = (os.environ.get("TMO_PRIOR_ROOT") or "").strip()
    if override:
        return Path(override)
    for candidate in CANDIDATE_PRIOR_ROOTS:
        if candidate.exists():
            return candidate
    return DEFAULT_PRIOR_ROOT
